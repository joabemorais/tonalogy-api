...
class ExplanationFormatter:
    ...
    def format_explanation(
        self, analysis: ProgressionAnalysisResponse, original_chords: Optional[List[str]] = None
    ) -> str:
        """
    ...
    def _build_introduction(self, analysis: ProgressionAnalysisResponse) -> str:
        ...
        if analysis.is_tonal_progression and self._main_tonality_cache:
            ...
            return T("explanation.formatter.intro_non_tonal", chord_sequence=chord_list)
    ...
    def _describe_functional_progression(self, steps: List[ExplanationStepAPI]) -> str:
        ...
        for step in steps:
            if step.processed_chord and step.evaluated_functional_state:
                ...
                chord_to_tonality[step.processed_chord] = (
                    step.tonality_used_in_step or current_tonality
                )
        ...
        main_description = self._build_functional_description(
            chord_functions, current_tonality or "unknown"
        )
                ...
                if i + 1 < len(self._chord_sequence_cache):
                    ...
                    if next_chord in chord_info:
                        ...
                        # Check if the next chord can be the target of this tonicization
                            ...
                            next_tonality == tonality and next_function in ["TONIC", "SUBDOMINANT"]
                        ) or (next_tonality == main_tonality and next_function == "SUBDOMINANT"):
    ...
    def _build_functional_description(
        self, chord_functions: List[Tuple[str, str, str]], main_tonality: str
    ) -> str:
        ...
        main_tonality_chords = [
            (chord, func)
            for chord, func, tonality in chord_functions
    ...
    def _group_by_tonality(
        self, steps: List[ExplanationStepAPI]
    ...
    def _describe_function_sequence(
        self, chord_functions: List[Tuple[str, str]], tonality: str
    ) -> str:
    ...
    def _describe_with_original_order(
        self, chord_functions: List[Tuple[str, str]], tonality: str
    ) -> str:
    ...
    def _identify_progression_patterns(
        self, chord_functions: List[Tuple[str, str]], tonality: str
    ) -> str:
    ...
    def _identify_all_cadences(self, chord_functions: List[Tuple[str, str]]) -> str:
        ...
        if cadences:
            ...
            if len(cadences) == 1:
                ...
                return T(
                    "explanation.formatter.two_cadences", first=cadences[0], second=cadences[1]
                )
            else:
                # Multiple cadences: use commas and "and" for the last one
                return T(
                    "explanation.formatter.multiple_cadences",
                    cadences_list=", ".join(cadences[:-1]),
                    last_cadence=cadences[-1],
                )
    ...
    def _describe_modulations(self, pivot_steps: List[ExplanationStepAPI]) -> str:
        ...
        for step in pivot_steps:
            if step.pivot_target_tonality and step.processed_chord:
                ...
                translated_target = translate_tonality(
                    step.pivot_target_tonality, locale_manager.current_locale
                )
                translated_current = (
                    translate_tonality(step.tonality_used_in_step, locale_manager.current_locale)
                    if step.tonality_used_in_step
                    else "unknown"
                )
    ...
    def _build_conclusion(self, analysis: ProgressionAnalysisResponse) -> str:
        ...
        if analysis.is_tonal_progression and self._main_tonality_cache:
            ...
            if has_tonicizations:
                ...
                return T(
                    "explanation.formatter.conclusion_tonal", tonality=self._main_tonality_cache
                )
        else:
    ...
    def _has_secondary_functions(self, steps: List[ExplanationStepAPI]) -> bool:
        ...
        for step in steps:
            if (
                ...
                and step.tonality_used_in_step
                and step.tonality_used_in_step != main_tonality
            ):
                return True
        return False
