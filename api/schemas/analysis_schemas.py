...
class ProgressionAnalysisRequest(BaseModel):
    ...
    tonalities_to_test: Optional[List[str]] = Field(
        ...
        json_schema_extra={"example": ["C Major", "A minor"]}
    )
...
class ExplanationStepAPI(BaseModel):
    ...
    """
    formal_rule_applied: Optional[str] = Field(None, description="The formal model rule that was applied.")
    observation: str = Field(..., description="A readable description of what happened in this step.")
    processed_chord: Optional[str] = Field(None, description="The chord that was processed in this step.")
    tonality_used_in_step: Optional[str] = Field(None, description="The tonality in use during this step.")
    evaluated_functional_state: Optional[str] = Field(None, description="The evaluated functional state (e.g., 'TONIC (s_t)').")
    ...
    def from_orm(cls, orm_obj):
        ...
        return cls(
            ...
            tonality_used_in_step=orm_obj.tonality_used_in_step.tonality_name if orm_obj.tonality_used_in_step else None,
            evaluated_functional_state=f"{orm_obj.evaluated_functional_state.associated_tonal_function.name} ({orm_obj.evaluated_functional_state.state_id})" if orm_obj.evaluated_functional_state else None
        )
