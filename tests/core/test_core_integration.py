...
def test_full_analysis_of_complex_progression(
    ...
    success, explanation = analyzer.check_tonal_progression(
        progression_to_analyze, tonalities_to_test
    )
    ...
    pivot_step = next(
        ...
        if "Pivot" in s.formal_rule_applied and "Eq.5" in s.formal_rule_applied
    )
